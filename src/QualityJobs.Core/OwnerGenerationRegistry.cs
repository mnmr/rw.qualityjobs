...
namespace QualityJobs.Core
{
    ...
    public sealed class OwnerGenerationRegistry<TOwner, TLocalKey, TLookupKey, TValue>
    {
        ...
        private sealed class Entry
        {
            ...
            internal Entry(TOwner owner, TLocalKey localKey, TLookupKey lookupKey, TValue value)
            {
                Owner = owner;
                LocalKey = localKey;
                LookupKey = lookupKey;
        ...
        public void Touch(TLocalKey localKey, TLookupKey lookupKey, TValue value)
        {
            ...
            {
                entry = AddEntry(activeOwner!, localKey, lookupKey, value);
        ...
        private Entry AddEntry(TOwner owner, TLocalKey localKey, TLookupKey lookupKey, TValue value)
        {
            var entry = new Entry(owner, localKey, lookupKey, value);
            entryCount++;
            if (!entriesByOwner.TryGetValue(owner, out HashSet<Entry>? owned))
            {
